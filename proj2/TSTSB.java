...
/**
 * A version of the authors TST class, but utilizing StringBuilder
...
public class TSTSB<Value> {
    ...
    *************************************************************
    ...
    private void collect(Node x, StringBuilder prefix, Queue<StringBuilder> queue) {
        ...
        if (x.val != null) {
            queue.enqueue(prefix.append(x.c));
        }
    ...
    public void collect(Node x, StringBuilder prefix, int i, StringBuilder pat, Queue<StringBuilder> q) {
        ...
        if (c == '.' || c == x.c) {
            if (i == pat.length() - 1 && x.val != null) {
                q.enqueue(prefix.append(x.c));
            }
            if (i < pat.length() - 1) {
                collect(x.mid, prefix.append(x.c), i + 1, pat, q);
            }
        }
        if (c == '.' || c > x.c) {
